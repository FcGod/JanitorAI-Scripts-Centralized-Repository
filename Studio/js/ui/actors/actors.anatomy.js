(function (root) {
    ...
    // tiny helpers
    function q (id) {
        return lastRoot ? lastRoot.querySelector ('#' + id) : null;
    }
    ...
    function buildHTML () {
        ...
        function sel (id, label, opts) {
            var i, h = '<label>' + label + '</label><select id="' + id + '">';
            for (i = 0; i < opts.length; i++) {
                h += '<option value="' + opts[i] + '">' + opts[i] + '</option>';
            }
            h += '</select>';
            return h;
        }
        ...
        function chk (id, txt) {
            return '<label><input id="' + id + '" type="checkbox"> ' + txt + '</label>';
        }

        function num (id, label, min, max, step) {
            return '<label>' + label + '</label><input id="' + id + '" type="number" min="' + min + '" max="' + max + '" step="' + step + '">';
        }

        function rng01 (id, label) {
            return '<label>' + label + '</label><input id="' + id + '" type="range" min="0" max="1" step="0.05">';
        }
    ...
    function readFromStore () {
        ...
        // markings/texture
        var m = ap.markings || [];
        var zone = q ('ap-markings');
        if (zone) {
            var i, h = '';
            for (i = 0; i < m.length; i++) {
                h += '<span class="chip">' + m[i] + '</span>';
            }
            zone.innerHTML = h || '<div class="muted">No markings.</div>';
        }
    ...
    function wireHandlers () {
        function onToggle (id, slot) {
            ...
            e.onchange = function () {
                ...
                patch (function (cfg) {
                    ...
                    if (!on) {
                        delete a.appearance.appendages[slot];
                    }
                    return cfg;
                });
                readFromStore ();
        ...
        function onSel (id, path) {
            var e = q (id);
            if (!e) return;
            e.onchange = function () {
                deepWrite (path, String (e.value || ''));
            };
        }

        function onRange01 (id, path) {
            var e = q (id);
            if (!e) return;
            e.oninput = function () {
                deepWrite (path, clamp01 (e.value));
            };
        }

        function onNum (id, path) {
            var e = q (id);
            if (!e) return;
            e.oninput = function () {
                var v = +e.value || 0;
        ...
        (function () {
            ...
            if (e) {
                e.onchange = function () {
                    deepWrite (['appearance', 'appendages', 'tail', 'prehensile'], !!e.checked);
                };
            }
        }) ();
        ...
        if (add && input) {
            add.onclick = function () {
                ...
                patch (function (cfg) {
                    var a = cfg.actors[activeId];
                    a.appearance.markings = a.appearance.markings || [];
                    if (a.appearance.markings.indexOf (v) < 0) a.appearance.markings.push (v);
                    return cfg;
                });
                input.value = '';
                readFromStore ();
        ...
        (function (id, key) {
            ...
            if (e) e.onchange = function () {
                patch (function (cfg) {
                    var a = cfg.actors[activeId];
                    a.appearance.texture = a.appearance.texture || {};
                    a.appearance.texture[key] = !!e.checked;
                    return cfg;
                });
            };
        }) ('ap-tex-fur', 'fur');
        ...
        }) ('ap-tex-feathers', 'feathers');
        var note = q ('ap-tex-notes');
        if (note) {
            note.oninput = function () {
                patch (function (cfg) {
                    var a = cfg.actors[activeId];
                    a.appearance.texture = a.appearance.texture || {};
                    a.appearance.texture.notes = String (note.value || '');
                    return cfg;
                });
            };
        }
