(function(root){
  ...
  // Store helpers
  // -----------------------------
  function getStore(){ return (root.CFGStore && CFGStore.get) ? CFGStore.get() : { actors:{}, relations:{} }; }
  ...
  function subscribeActorsAndRelations(cb){
    if (root.CFGStore && CFGStore.subscribe){
      CFGStore.subscribe(function(){
        ...
        } catch(_e){ return (new Date()).getTime(); }
      }, function(){ cb && cb(); });
    }
  }
  ...
  function valence(entry){
    var pos = (entry.trust + entry.affinity + entry.respect) / 3;
    var v = pos - entry.rivalry; if (v<-1) v=-1; if (v>1) v=1;
    return v;
  }
  function colorFor(v){
    var r,g,b;
  ...
  function aggregateForGraph(fromId){
    ...
    for (k in rel){
      ...
      for (i=0;i<n;i++){
        acc.trust+=E[i].trust||0; acc.affinity+=E[i].affinity||0; acc.respect+=E[i].respect||0;
        acc.rivalry+=E[i].rivalry||0; acc.influence+=E[i].influence||0;
      }
  ...
  function drawCompass(canvas, centerId){
    ...
    for (i=1;i<=3;i++){ ctx.beginPath(); ctx.arc(cx,cy,Rbase*(i/3),0,Math.PI*2,true); ctx.stroke(); }

    ctx.fillStyle = 'rgb(90,210,190)';
    ctx.beginPath(); ctx.arc(cx,cy,10,0,Math.PI*2,true); ctx.fill();
    ...
    for (i=0;i<N;i++){
      ...
      ctx.fillStyle = color;
      ctx.beginPath(); ctx.arc(x,y,nodeSize,0,Math.PI*2,true); ctx.fill();
  ...
  function computeStatus(id){
    ...
    for (k in rel){ if (rel.hasOwnProperty(k) && rel[k] && (rel[k].entries||[]).length) count++; }
  ...
  function buildEditorHTML(){
    ...
    function sel(id, arr, ph, labeller){
      var h = '<select id="'+id+'">'; if (ph) h += '<option value="">'+ph+'</option>';
      for (var j=0;j<arr.length;j++){
        var v = arr[j]; h += '<option value="'+v+'">'+(labeller?labeller(v):v)+'</option>';
      }
      h += '</select>'; return h;
    }
    function presetBar(){
  ...
  function renderSidebar(){
    ...
    for (i=0;i<cards.length;i++){
      cards[i].onclick = (function(el){ return function(){
        activeId = el.getAttribute('data-id'); renderSidebar(); renderEditor();
      };})(cards[i]);
    }
  }
  ...
  function renderEditor(){
    ...
    if (!activeId){ ed.innerHTML = '<div class="muted">Select an actor to edit relationships.</div>'; return; }
  ...
  function drawLedger(){
    ...
    if (!rows.length){ zone.innerHTML = '<div class="muted">No relationships yet.</div>'; return; }
  ...
  function fmt(v){ return (Math.round(clamp01(v)*100))+'%'; }

  // -----------------------------
  // Editor data flow
  // -----------------------------
  function getSelectedTargetId(){
    var el = lastRoot.querySelector('#rel-target'); return el ? el.value : '';
  }
  function setSelectedTargetId(toId){
    var el = lastRoot.querySelector('#rel-target'); if (el){ el.value = toId||''; }
  }
  function setSelectedType(type){
    var el = lastRoot.querySelector('#rel-type'); if (el){ el.value = type||''; }
  }
  function setSlider(id, v){
    var el = lastRoot.querySelector('#'+id); if (el){ el.value = Math.round(clamp01(v)*100); }
    var pct = lastRoot.querySelector('#'+id+'-pct'); if (pct){ pct.textContent = Math.round(clamp01(v)*100)+'%'; }
  }
  function getSliderVal(id){
    var el = lastRoot.querySelector('#'+id); return el ? Math.max(0,Math.min(100,parseInt(el.value,10)||0))/100 : 0;
  }
  ...
  function applyPreset(pkey){
    var p = PRESETS[pkey] || null; if (!p) return;
  ...
  function addOrUpdateEntry(reciprocal){
    var toId = getSelectedTargetId(); if (!toId) return;
    ...
    if (reciprocal){
      patch(function(cfg){
        ...
        var inv = cfg.relations[toId][activeId] = cfg.relations[toId][activeId] || { entries:[], user_frame:'' };
        var mir = {
          type: entry.type,
          trust: entry.trust,
  ...
  function fillEditorFromCurrent(){
    ...
    for (i=0;i<ids.length;i++){ if (ids[i]!==activeId){ firstOther = ids[i]; break; } }
  ...
  function bindEditorEvents(){
    ...
    if (clr){
      clr.onclick = function(){
        var to = getSelectedTargetId(); if (!to) return;
    ...
    for (i=0;i<presets.length;i++){
      presets[i].onclick = (function(btn){ return function(){ applyPreset(btn.getAttribute('data-p')); }; })(presets[i]);
    }
    ...
    if (headLine){ headLine.appendChild(addBtn); }
    addBtn.onclick = function(){ addOrUpdateEntry(recip && recip.checked); };
  }
  ...
  api.mount = function(rootEl){
    ...
    subscribeActorsAndRelations(function(){
      if (!lastRoot) return;
      if (!activeId){
        var now = actorIds(); if (now.length) activeId = now[0];
      }
      renderSidebar();
  ...
  api.unmount = function(){ if (lastRoot) lastRoot.innerHTML=''; lastRoot = null; };
