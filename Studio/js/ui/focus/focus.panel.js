(function (root) {
    ...
    // ---- store helpers ----
    function getStore () {
        return (root.CFGStore && CFGStore.get) ? CFGStore.get () : {actors: {}, params: {}};
    }
    ...
    function render () {
        ...
        totalInput.oninput = function () {
            ...
            var i, sum = 0;
            for (i = 0; i < Local.actors.length; i++) sum += (Local.actors[i].value | 0);
            if (sum > 0) {
                for (i = 0; i < Local.actors.length; i++) {
        ...
        for (i = 0; i < rowsEl.length; i++) {
            (function (row) {
                ...
                function applyDisabled () {
                    var dis = !!a.locked;
                    if (slider) slider.disabled = dis;
                    if (num) num.disabled = dis;
                }
                ...
                if (slider) {
                    slider.oninput = function () {
                        ...
                        if (aNow) {
                            num.value = aNow.value | 0;
                            pctEl.textContent = (Local.total ? Math.round ((aNow.value / Local.total) * 100) : 0) + '%';
                        }
                ...
                if (num) {
                    num.oninput = function () {
                        ...
                        if (aNow) {
                            pctEl.textContent = (Local.total ? Math.round ((aNow.value / Local.total) * 100) : 0) + '%';
                            slider.value = (Local.total ? Math.round ((aNow.value / Local.total) * 100) : 0);
                        }
                        __syncOthersExcept (id);
                        writeToCFG ();
                        updateRadar ();
    ...
    function findLocal (id) {
        var i;
        for (i = 0; i < Local.actors.length; i++) if (Local.actors[i].id === id) return Local.actors[i];
        return null;
    }
