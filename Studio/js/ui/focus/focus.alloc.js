(function (root) {
    ...
    function sumValues (list) {
        var s = 0, i;
        for (i = 0; i < list.length; i++) s += Math.max (0, list[i].value | 0);
        return s;
    }
    ...
    function unlockedOthers (list, skipId) {
        ...
        for (i = 0; i < list.length; i++) {
            a = list[i];
            if (!a.locked && a.id !== skipId) out.push (a);
        }
        return out;
    }
    ...
    Alloc.setValueAndRebalance = function (state, actorId, newValue) {
        ...
        var pool = 0;
        for (i = 0; i < others.length; i++) pool += Math.max (0, others[i].value | 0);
    ...
    Alloc.equalize = function (state) {
        var list = state.actors, unlocked = [], i, lockedSum = 0;
        for (i = 0; i < list.length; i++) {
            var a = list[i];
            if (a.locked) lockedSum += (a.value | 0); else unlocked.push (a);
        }
        if (!unlocked.length) return;

        var total = state.total | 0;
        var budget = total - lockedSum;
        if (budget < 0) budget = 0;

        var base = Math.floor (budget / unlocked.length);
        var rem = budget - base * unlocked.length;

        // Set unlocked rows to base
        for (i = 0; i < unlocked.length; i++) {
            unlocked[i].value = base;
        }

        // Distribute remainder round-robin across unlocked only
        var idx = 0, guard = 10000;
        while (rem > 0 && guard--) {
            unlocked[idx].value += 1;
            rem--;
    ...
    Alloc.biasActive = function (state, activeId, bias) {
        ...
        for (i = 0; i < A.length; i++) if (A[i].id === activeId) {
            target = A[i];
            break;
        }
        if (!target) return;
