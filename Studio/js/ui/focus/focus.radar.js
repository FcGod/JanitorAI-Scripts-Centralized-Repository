(function (root) {
    ...
    function normalizeWeights (actors) {
        ...
        if (!sum) {
            var even = 1 / (actors.length || 1);
            for (i = 0; i < actors.length; i++) actors[i]._pct = even;
            return actors;
        }
    ...
    function ringColor (op) {
        return rgba (77, 124, 255, op);
    }
    ...
    function computeRingDensities (points, maxR, minR, ringCount) {
        ...
        for (i = 0; i < points.length; i++) {
            ...
            if (band < 0) band = 0;
            if (band >= ringCount) band = ringCount - 1;
            bands[band] += 1;
        }
        var max = 0;
        for (i = 0; i < ringCount; i++) if (bands[i] > max) max = bands[i];
        if (!max) return bands;
    ...
    function makeTooltip () {
        ...
        t.style.transform = 'translate(8px,-8px)';
    ...
    function hitTest (points, mx, my, pad) {
        var i;
        for (i = 0; i < points.length; i++) {
            var p = points[i], dx = mx - p.x, dy = my - p.y;
            if ((dx * dx + dy * dy) <= pad * pad) return p;
        }
        return null;
    }

    function render (state) {
        var c = state.ctx, w = state.w, h = state.h, cx = state.cx, cy = state.cy, maxR = state.maxR, minR = state.minR,
            rings = state.ringCount, pts = state.points;
        c.fillStyle = bgColor ();
        c.fillRect (0, 0, w, h);
        ...
        for (i = 0; i < rings; i++) {
            ...
            c.beginPath ();
            c.arc (cx, cy, rO, 0, TWO_PI, false);
            c.arc (cx, cy, rI, 0, TWO_PI, true);
            c.closePath ();
            c.fillStyle = ringColor (op);
            c.fill ();
            c.beginPath ();
            c.arc (cx, cy, rO, 0, TWO_PI, false);
        ...
        for (i = 0; i < axes; i++) {
            ...
            c.beginPath ();
            c.moveTo (cx + Math.cos (th) * minR, cy + Math.sin (th) * minR);
        ...
        for (i = 0; i < pts.length; i++) {
            c.beginPath ();
            c.arc (pts[i].x, pts[i].y, 4, 0, Math.PI * 2, false);
            c.fill ();
        }
        ...
        for (i = 0; i < pts.length; i++) {
            var p = pts[i];
            c.fillText (p.name + ' (' + Math.round (p.pct * 100) + '%)', p.x + 8, p.y);
        }
    ...
    FocusRadar.mount = function (el, opts) {
        ...
        canvas.width = (opts && opts.width) || 560;
        canvas.height = (opts && opts.height) || 340;
        ...
        var rings = (opts && opts.rings) || 6,
            minRatio = (opts && opts.minRadiusRatio != null) ? opts.minRadiusRatio : 0.12;
        ...
        canvas.onmousemove = function (ev) {
            var r = canvas.getBoundingClientRect (), mx = ev.clientX - r.left, my = ev.clientY - r.top,
                hit = hitTest (state.points, mx, my, 8);
            if (hit) {
                tip.style.left = (mx + 12) + 'px';
                tip.style.top = (my - 12) + 'px';
                tip.innerHTML = hit.name + '<br>' + Math.round (hit.pct * 100) + '% (' + (hit.weight || 0) + 'w)';
    ...
    FocusRadar.update = function (el, actors) {
        var st = el && el.__focusRadar;
        if (!st) return;
        st.actors = (actors || []).slice ();
        normalizeWeights (st.actors);
        st.points = (function (a) {
            return (a && a.length) ? (function () {
                return (function () {
                    return (function () {
                        return (function () {
                            return (function cx () {
        ...
        } (st.actors), layoutPoints (st.actors, st.cx, st.cy, st.maxR, st.minR));
