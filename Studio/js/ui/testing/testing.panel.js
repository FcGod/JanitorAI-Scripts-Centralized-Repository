(function(root){
  ...
  function renderScenarios(){
    ...
    if (!items.length){ node.innerHTML = '<div class="muted">No scenarios.</div>'; return; }
    var i, html='';
  ...
  function drawInspector(run){
    ...
    if (idx<0 || !run.messages[idx] || !run.messages[idx].why){
      ins.innerHTML = '<div class="muted">Select an actor line to inspect.</div>'; return;
    }
  ...
  function bindControls(){
    ...
    if (seed){ seed.oninput = function(){ state.seed = parseInt(seed.value, 10) || 1337; }; }
    if (turns){ turns.oninput = function(){ state.turns = Math.max(3, Math.min(24, parseInt(turns.value,10)||6)); }; }
    if (runBtn){ runBtn.onclick = executeRun; }
    ...
    if (root.CFGStore && CFGStore.subscribe){
      CFGStore.subscribe(function(){
        ...
        try { return JSON.stringify({ actors:s.actors||{}, params:s.params||{} }); }
  ...
  function executeRun(){
    var run = (root.TestRunner && TestRunner.runScenario) ? TestRunner.runScenario(state.scenarioId, { seed:state.seed, turns:state.turns }) : null;
    if (!run) return;
