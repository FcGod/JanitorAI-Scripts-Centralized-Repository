(function (root) {
    ...
    function sampleQuirk (a, rnd) {
        ...
        function push (arr) {
            if (!arr) return;
            for (i = 0; i < arr.length; i++) all.push (arr[i]);
        }

        push (banks.physical);
        push (banks.mental);
        push (banks.emotional);
    ...
    function runScenario (scenarioId, opts) {
        opts = opts || {};
        var cfg = getStore ();
        var scenario = (root.TestScenarios && TestScenarios.get (scenarioId)) || {
            id: 'custom',
            title: 'Custom',
            tone: 'neutral',
            text: ''
        ...
        // Totals
        var totalTokens = 0, j;
        for (j = 0; j < out.length; j++) totalTokens += out[j].tokens | 0;
