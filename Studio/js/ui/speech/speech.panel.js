(function(root){
  ...
  // Store helpers
  // -----------------------------
  function getStore(){ return (root.CFGStore && root.CFGStore.get) ? root.CFGStore.get() : { actors:{} }; }
  function patch(mut){ if (root.CFGStore && root.CFGStore.patch) CFGStore.patch(mut); }
  ...
  function computeStatus(a){
    ...
    if (a.cues_body_parts){
      ...
      for (i2=0;i2<kk.length;i2++){
        ...
        if (p && (p.ears||p.tail||p.wings||p.horns)) { score++; break; }
      }
    }
  ...
  // -----------------------------
  function getCompactPref(){ try { return localStorage.getItem('MYTHOS_SPEECH_COMPACT') === '1'; } catch(_e){} return false; }
  function setCompactPref(on){ try { localStorage.setItem('MYTHOS_SPEECH_COMPACT', on ? '1' : '0'); } catch(_e){} }
  ...
  function ensureCueDefaultsFor(aId){
    patch(function(cfg){
      var a = (cfg.actors && cfg.actors[aId]) || null; if(!a) return;
      if (!a.cues) a.cues = {};
      var keys = cuesKeys(), defs = cuesDefaults(), i, k;
      for (i=0;i<keys.length;i++){ k=keys[i]; if (!a.cues[k]) a.cues[k] = defs[k] || ''; }
  ...
  function ensureBodyCueDefaultsFor(aId){
    patch(function(cfg){
      ...
      for (i=0;i<keys.length;i++){ k=keys[i]; if (a.cues_body[k] == null) a.cues_body[k] = ''; }
      return cfg;
    });
  }
  ...
  function initCompactToolbar(rootEl){
    var right = rootEl.querySelector('.speech-col-right'); if (!right) return;
    var bar = document.createElement('div'); bar.className = 'speech-toolbar';
    ...
    var compactOn = getCompactPref(); if (compactOn) { editor.className += ' compact'; }
    ...
    if (tBtn){
      ...
      tBtn.onclick = function(){
        ...
        var nowOn = !has; setCompactPref(nowOn);
  ...
  function buildSidebarHTML(){
    var c = getStore(), ids = [], k; for (k in (c.actors||{})) ids.push(k); ids.sort();
  ...
  function renderSidebar(){
    var el = lastRoot.querySelector('.actor-list'); if (!el) return;
    el.innerHTML = buildSidebarHTML();
    var cards = el.querySelectorAll('.actor-card'); var i;
    for (i=0;i<cards.length;i++){
      (function(card){ card.onclick = function(){ var id = card.getAttribute('data-id'); if (id){ setActive(id); } }; })(cards[i]);
    }
  }
  ...
  function renderEditor(){
    var editor = lastRoot.querySelector('.speech-editor'); if (!editor) return;
    editor.innerHTML = buildEditorHTML();

    var s = getStore(); var a = (s.actors||{})[activeId] || null; if (!a) return;
  ...
  function bindBase(editor){
    ...
    if (toneSel) toneSel.onchange = function(){ patch(function(cfg){ var a=cfg.actors[activeId]; a.speech=a.speech||{}; a.speech.tone = String(toneSel.value||''); return cfg; }); updateMeters(editor); };
    if (pacSel)  pacSel.onchange  = function(){ patch(function(cfg){ var a=cfg.actors[activeId]; a.speech=a.speech||{}; a.speech.pacing = String(pacSel.value||''); return cfg; }); updateMeters(editor); };
    if (verSel)  verSel.onchange  = function(){ patch(function(cfg){ var a=cfg.actors[activeId]; a.speech=a.speech||{}; a.speech.verbosity = String(verSel.value||''); return cfg; }); updateMeters(editor); };
    if (dicIn)   dicIn.oninput    = function(){ patch(function(cfg){ var a=cfg.actors[activeId]; a.speech=a.speech||{}; a.speech.diction = String(dicIn.value||''); return cfg; }); };
    if (snapIn)  snapIn.oninput   = function(){ patch(function(cfg){ var a=cfg.actors[activeId]; a.summary=a.summary||{}; a.summary.line = String(snapIn.value||''); return cfg; }); };
  }

  function updateMeters(editor){
    function fill(id, v){ var el = editor.querySelector('#'+id); if (!el) return; el.style.width = (has(v) ? 100 : 12) + '%'; }
    var s = getStore(), a = (s.actors||{})[activeId] || {}, sp = a.speech||{};
    fill('m-tone', sp.tone); fill('m-pacing', sp.pacing); fill('m-verbosity', sp.verbosity);
  }
  ...
  function renderQuirkList(editor, kind, list){
    var zone = editor.querySelector('#q-'+kind+'-list'); if (!zone) return;
    var i, h=''; for (i=0;i<list.length;i++){ h += '<span class="chip" data-kind="'+kind+'" data-idx="'+i+'">'+list[i]+' <b class="x">×</b></span>'; }
  ...
  function bindQuirks(editor){
    ...
    for (i=0;i<kinds.length;i++){
      (function(kind){
        ...
        if (addBtn){ addBtn.onclick = function(){
          ...
          val = (val||'').replace(/^\s+|\s+$/g,''); if (!val) return;
          patch(function(cfg){ var a = cfg.actors[activeId]; a.quirks = a.quirks || { physical:[], mental:[], emotional:[] }; var arr = a.quirks[kind] || (a.quirks[kind]=[]); if (arr.indexOf(val) < 0) arr.push(val); return cfg; });
          renderEditor();
        }; }
      })(kinds[i]);
    }
    editor.addEventListener('click', function(ev){
      ...
      if (t.className === 'x' && t.parentNode && t.parentNode.className.indexOf('chip')>-1){
        var chip = t.parentNode; var kind = chip.getAttribute('data-kind'); var idx = parseInt(chip.getAttribute('data-idx'),10); if (isNaN(idx)) return;
        patch(function(cfg){ var a = cfg.actors[activeId]; a.quirks = a.quirks || { physical:[], mental:[], emotional:[] }; var arr = a.quirks[kind] || []; if (idx>=0 && idx<arr.length) arr.splice(idx,1); return cfg; });
        renderEditor();
      }
    }, false);
  }
  ...
  function fillCueTable(editor, a){
    ...
    for (i=0;i<keys.length;i++){
      k = keys[i];
      var row = editor.querySelector('tr[data-em="'+k+'"]'); if (!row) continue;
      var basic = row.querySelector('#cue-'+k); if (basic) basic.value = a.cues[k] || '';
  ...
  function bindCueTable(editor){
    ...
    for (i=0;i<keys.length;i++){
      (function(em){
        var el = editor.querySelector('#cue-'+em); if (!el) return;
        el.oninput = function(){ patch(function(cfg){ var a = cfg.actors[activeId]; a.cues = a.cues || {}; a.cues[em] = String(el.value||''); return cfg; }); };
      })(keys[i]);
    }
    // Parts columns → a.cues_body_parts[k].(ears|tail|wings|horns)
    function bindPart(em, part){
      var id = '#cpart-'+part+'-'+em, el = editor.querySelector(id); if (!el) return;
      el.oninput = function(){ patch(function(cfg){ var a = cfg.actors[activeId]; a.cues_body_parts = a.cues_body_parts || {}; a.cues_body_parts[em] = a.cues_body_parts[em] || { ears:'', tail:'', wings:'', horns:'' }; a.cues_body_parts[em][part] = String(el.value||''); return cfg; }); };
    }
    for (i=0;i<keys.length;i++){ k = keys[i]; bindPart(k,'ears'); bindPart(k,'tail'); bindPart(k,'wings'); bindPart(k,'horns'); }
  }
  ...
  api.mount = function(el){
    if (!el) return; lastRoot = el; buildShell(el); initCompactToolbar(el);
    renderSidebar();
    var s = getStore(), ids = [], k; for (k in (s.actors||{})) ids.push(k);
    if (ids.length){ setActive(ids[0]); }
    else { var ed = el.querySelector('.speech-editor'); if (ed) ed.innerHTML = '<div class="muted">Create an actor in the Actors panel to begin.</div>'; }
  };

  api.update = function(){
    if (!lastRoot) return; renderSidebar();
    if (activeId){ ensureCueDefaultsFor(activeId); ensureBodyCueDefaultsFor(activeId); ensureBodyPartsDefaultsFor(activeId); renderEditor(); }
  };
