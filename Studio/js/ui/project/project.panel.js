(function (root) {
    ...
    function getStore () {
        return (root.CFGStore && root.CFGStore.get) ? root.CFGStore.get () : {};
    }

    function patch (fn) {
        if (root.CFGStore && root.CFGStore.patch) root.CFGStore.patch (fn);
    }
    ...
    api.mount = function (el) {
        ...
        // bindings → store
        if (projTitle) projTitle.oninput = function () {
            patch (function (c) {
                c.project = c.project || {};
                c.project.title = projTitle.value;
            });
        };
        if (projAuthor) projAuthor.oninput = function () {
            patch (function (c) {
                c.project = c.project || {};
                c.project.author = projAuthor.value;
            });
        };
        if (debug) debug.onchange = function () {
            patch (function (c) {
                c.features = c.features || {};
                c.features.debug = !!debug.checked;
            });
        };
        if (notesG) notesG.oninput = function () {
            patch (function (c) {
                c.meta = c.meta || {};
                c.meta.notes = c.meta.notes || {};
                c.meta.notes.general = notesG.value;
            });
        };
        if (notesS) notesS.oninput = function () {
            patch (function (c) {
                c.meta = c.meta || {};
                c.meta.notes = c.meta.notes || {};
                c.meta.notes.scenario = notesS.value;
            });
        };
        if (notesP) notesP.oninput = function () {
            patch (function (c) {
                c.meta = c.meta || {};
                c.meta.notes = c.meta.notes || {};
                c.meta.notes.personality = notesP.value;
            });
        };

        // save/load/clear
        if (saveBtn) saveBtn.onclick = function () {
            if (CFGStore && CFGStore.saveProfile) CFGStore.saveProfile ();
            if (statusEl) statusEl.textContent = 'Profile saved.';
        };
        if (loadBtn) loadBtn.onclick = function () {
            if (CFGStore && CFGStore.loadProfile) CFGStore.loadProfile (function (cfg) {
                setFieldsFrom (cfg || CFGStore.get ());
            });
        };
        if (clearBtn) clearBtn.onclick = function () {
            if (CFGStore && CFGStore.clearProfile) CFGStore.clearProfile ();
        ...
        if (expCfg) expCfg.onclick = function () {
            compile ('cfg');
        };
        if (expLore) expLore.onclick = function () {
            compile ('lorebook');
        };
        if (expCart) expCart.onclick = function () {
            compile ('cartridge', {
        ...
        if (root.CFGStore && CFGStore.subscribe) {
            CFGStore.subscribe (function (cfg) {
                setFieldsFrom (cfg);
            });
        }
    };
