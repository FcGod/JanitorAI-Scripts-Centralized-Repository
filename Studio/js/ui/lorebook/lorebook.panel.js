...
(function (root) {
    ...
    function assign (t, s) {
        ...
        if (!t) t = {};
        if (!s) return t;
        for (k in s) if (s.hasOwnProperty (k)) t[k] = s[k];
        return t;
    }
    ...
    function off (elm, ev, fn) {
        elm.removeEventListener (ev, fn, false);
    }
    ...
    var Store = (function () {
        ...
        function patch (delta) {
            ...
            if (root.CFGStore && typeof root.CFGStore.patch === 'function') {
                try {
                    return root.CFGStore.patch (function (c) {
                        cfg = c || {};
                        var k;
                        for (k in delta) if (delta.hasOwnProperty (k)) cfg[k] = delta[k];
                        return cfg;
                    });
                } catch (_e) {
                    try {
                        return root.CFGStore.patch (delta);
                    } catch (_e2) {
                    }
                }
            }
            cfg = get ();
            var k2;
            for (k2 in delta) if (delta.hasOwnProperty (k2)) cfg[k2] = delta[k2];
    ...
    function normalizeEntry (raw, usedUuids) {
        ...
        if (!e.uuid || typeof e.uuid !== 'string') {
            e.uuid = genUniqueUuid (usedUuids);
        }
    ...
    function normalizeAll (list) {
        ...
        for (i = 0; i < list.length; i++) {
            var n = normalizeEntry (list[i], uu);
            out.push (n);
            uu[n.uuid] = 1;
        }
        return out;
    ...
    function genUniqueUuid (used) {
        var u = uuidv4 ();
        while (used && used[u]) u = uuidv4 ();
        return u;
    }
    ...
    function upsertByName (entries, name, patchObj) {
        ...
        for (i = 0; i < entries.length; i++) {
            if ((entries[i].name || '') === name) {
                assign (entries[i], patchObj);
                return entries[i];
            }
        }
        var used = {};
        for (i = 0; i < entries.length; i++) {
            used[entries[i].uuid] = 1;
        }
    ...
    function seedDescriptorsLocal (cfg) {
        ...
        for (id in A) {
            ...
            (function (aid, a) {
                ...
                var e1 = upsertByName (entries, aid + ':Base:Appearance', {
                    category: baseCat,
                    content: summarizeAppearance (a),
                    priority: 50,
                    probability: 100,
                    enabled: true
                });
                var sp = a.speech || {}, bits = [];
                if (exists (sp.tone)) bits.push ('tone:' + sp.tone);
                if (exists (sp.pacing)) bits.push ('pacing:' + sp.pacing);
                if (exists (sp.verbosity)) bits.push ('verbosity:' + sp.verbosity);
                if (exists (sp.diction)) bits.push ('diction:' + sp.diction);
                var e2 = upsertByName (entries, aid + ':Base:Voice', {
                    category: baseCat,
                    content: bits.join (' • '),
                    priority: 50,
                    probability: 100,
        ...
        for (id in A) {
            ...
            (function (aid, a) {
                ...
                for (i = 0; i < keys.length; i++) {
                    ...
                    if ((ad.ears && ad.ears.present) || exists (txtE)) {
                        if (exists (txtE)) added.push (upsertByName (entries, aid + ':' + Em + ':Ears', {
                            category: 'character',
                            content: txtE,
                            priority: 55,
                            probability: 100,
                    ...
                    if ((ad.tail && ad.tail.present) || exists (txtT)) {
                        if (exists (txtT)) added.push (upsertByName (entries, aid + ':' + Em + ':Tail', {
                            category: 'character',
                            content: txtT,
                            priority: 55,
                            probability: 100,
                    ...
                    if ((ad.wings && ad.wings.present) || exists (txtW)) {
                        if (exists (txtW)) added.push (upsertByName (entries, aid + ':' + Em + ':Wings', {
                            category: 'character',
                            content: txtW,
                            priority: 55,
                            probability: 100,
                    ...
                    if ((ad.horns && ad.horns.present) || exists (txtH)) {
                        if (exists (txtH)) added.push (upsertByName (entries, aid + ':' + Em + ':Horns', {
                            category: 'character',
                            content: txtH,
                            priority: 55,
                            probability: 100,
    ...
    function seedDescriptors (cfg) {
        return (LoreSeed && LoreSeed.seedActorDescriptors) ? LoreSeed.seedActorDescriptors (cfg) : seedDescriptorsLocal (cfg);
    }

    function seedEmotionCues (cfg) {
        return (LoreSeed && LoreSeed.seedEmotionCues) ? LoreSeed.seedEmotionCues (cfg) : seedEmotionCuesLocal (cfg);
    }
    ...
    function importEntries (rawText, existing, overwrite) {
        var list;
        var added = 0, overwritten = 0, skipped = 0, errors = 0;
        var msg = '';
        try {
        ...
        return {
            entries: existing,
            report: {added: added, overwritten: overwritten, skipped: skipped, errors: errors, note: msg}
        };
    }

    function exportEntries (entries, compact) {
        var blob = {entries: arr (entries)};
    ...
    function readEntriesFromCfg (cfg) {
        var list = (((cfg || {}).lorebook || {}).entries) || [];
        return (LoreModel && LoreModel.normalizeAll) ? LoreModel.normalizeAll (arr (list)) : normalizeAll (arr (list));
    }

    function writeEntriesToCfg (entries) {
        var cfg = Store.get ();
        var lb = cfg.lorebook || {};
        lb.entries = arr (entries);
        Store.patch ({lorebook: lb});
    }

    function signature (cfg) {
        var n = (cfg && cfg.lorebook && cfg.lorebook.entries && cfg.lorebook.entries.length) || 0;
        return [n, state.selectedUuid, state.searchBy, state.searchQuery, state.compact].join ('|');
    }
    ...
    function buildSkeleton (container) {
        ...
        var chkOverwriteLbl = el ('label', 'lb-check');
        var chkOverwrite = document.createElement ('input');
        chkOverwrite.type = 'checkbox';
        chkOverwriteLbl.appendChild (chkOverwrite);
        chkOverwriteLbl.appendChild (document.createTextNode (' overwrite'));
        var btnExport = el ('button', 'btn acc', 'Export JSON');
        var chkCompactLbl = el ('label', 'lb-check');
        var chkCompact = document.createElement ('input');
        chkCompact.type = 'checkbox';
        chkCompactLbl.appendChild (chkCompact);
        chkCompactLbl.appendChild (document.createTextNode (' compact'));

        // NEW: presence filter toggle (defaults ON)
        var chkPresenceLbl = el ('label', 'lb-check');
        var chkPresence = document.createElement ('input');
        chkPresence.type = 'checkbox';
        chkPresence.checked = true;
        chkPresenceLbl.appendChild (chkPresence);
        chkPresenceLbl.appendChild (document.createTextNode (' honor anatomy presence'));
        ...
        searchWrap.appendChild (selField);
        searchWrap.appendChild (txtSearch);
        ...
        on (btnImpF, 'click', function () {
            pickFile (function (text) {
                doImport (text);
            });
        });
        on (btnImpP, 'click', function () {
            var t = prompt ('Paste lorebook JSON (array or {entries})');
            if (typeof t === 'string') doImport (t);
        });
        ...
        on (chkOverwrite, 'change', function () {
            state.overwrite = !!chkOverwrite.checked;
        });
        on (chkCompact, 'change', function () {
            state.compact = !!chkCompact.checked;
        });
        on (selField, 'change', function () {
            state.searchBy = selField.value || 'name';
            renderList ();
        });
        on (txtSearch, 'input', function () {
            state.searchQuery = txtSearch.value || '';
            renderList ();
        });
    }
    ...
    function onAdd () {
        var used = {};
        var i;
        for (i = 0; i < state.entries.length; i++) used[state.entries[i].uuid] = 1;
        var e = normalizeEntry ({
            uuid: genUniqueUuid (used),
            category: 'uncategorized',
            name: 'New Entry',
            content: ''
        }, used);
        state.entries.push (e);
        state.selectedUuid = e.uuid;
        state.buffer = clone (e);
        commitRender ();
    }

    function onSeedDescriptors () {
        var cfg = Store.get ();
        var adds = seedDescriptors (cfg) || [];
        // Merge by name (idempotent):
        var i;
        for (i = 0; i < adds.length; i++) {
            upsertByName (state.entries, adds[i].name || ('seed-' + i), adds[i]);
        }
        persistAndRender ();
    }
    ...
    function renderList () {
        ...
        for (i = 0; i < filtered.length; i++) {
            ...
            if (state.selectedUuid === e.uuid && state.dirty) {
                var dot = el ('span', 'lb-pill', '\u25CF');
            ...
            if (parsed && facetIsAppendage (parsed.facet)) {
                ...
                if (!isOn) {
                    var off = el ('span', 'lb-pill warn', 'OFF');
                    off.title = 'Appendage not enabled in Actors → Anatomy';
                    b.appendChild (off);
                }
            }
            row.appendChild (a);
    ...
    function filterEntries (list, field, q) {
    ...
    function parseAKey (name) {
        if (!name) return null;
        var parts = String (name).split (':');
        if (parts.length < 3) return null;
        return {actor: parts[0], state: parts[1], facet: parts[2]};
    }

    function filterByAnatomyPresence (entries, cfg) {
        var map = presenceMap (cfg);
        var out = [], i, e, parsed, actor, facet, ok;
        for (i = 0; i < entries.length; i++) {
            e = entries[i];
            parsed = parseAKey (e && e.name);
            if (!parsed) {
                out.push (e);
                continue;
            }
    ...
    function onSelect (uuid) {
        state.selectedUuid = uuid;
        state.buffer = clone (findByUuid (state.entries, uuid));
        state.dirty = false;
        renderList ();
        renderEditor ();
    }
    ...
    function findByUuid (list, uuid) {
        var i;
        for (i = 0; i < list.length; i++) if (list[i].uuid === uuid) return list[i];
        return null;
    }
    ...
    function renderEditor () {
        ...
        empty (ed);
        if (!state.buffer) {
            ed.appendChild (el ('div', 'lb-empty', 'No entry selected.'));
            return;
        }
        var e = state.buffer;
        ...
        function row (label, inputEl) {
            var r = el ('div', 'lb-field');
            var L = el ('label', 'lb-label', label);
            r.appendChild (L);
        ...
        // Immutable UUID
        var uuidRO = input ('text', e.uuid);
        uuidRO.readOnly = true;
        form.appendChild (row ('UUID', uuidRO));

        var selCat = select (Categories, e.category);
        form.appendChild (row ('Category', selCat));
        var name = input ('text', e.name);
        form.appendChild (row ('Name', name));
        var content = textarea (e.content);
        form.appendChild (row ('Content', content));

        // Counters (via Metrics when available)
        var counter = el ('div', 'lb-counter', '0 words • ~0 tokens');
        form.appendChild (counter);

        var keys = input ('text', e.keysRaw);
        form.appendChild (row ('Keys', keys));
        var key2 = input ('text', e.keysecondaryRaw);
        form.appendChild (row ('Fallback Keys', key2));
        var keyPri = checkbox (e.keyMatchPriority);
        form.appendChild (row ('Key Priority', keyPri));
        var whole = checkbox (e.matchWholeWords);
        form.appendChild (row ('Match Whole Words', whole));
        var csens = checkbox (e.case_sensitive);
        form.appendChild (row ('Case Sensitive', csens));

        var actMode = select (Activation, e.activationMode);
        form.appendChild (row ('Activation Mode', actMode));
        var actScript = textarea (e.activationScript);
        form.appendChild (row ('Activation Script', actScript));

        var grp = input ('text', e.inclusionGroupRaw);
        form.appendChild (row ('Group (raw)', grp));
        var grpW = input ('number', e.groupWeight);
        form.appendChild (row ('Group Weight', grpW));
        var ins = input ('number', e.insertion_order);
        form.appendChild (row ('Insertion Order', ins));

        var prio = input ('number', e.priority);
        form.appendChild (row ('Priority', prio));
        var prob = input ('number', e.probability);
        prob.min = 0;
        prob.max = 100;
        form.appendChild (row ('Probability %', prob));

        var en = checkbox (e.enabled);
        form.appendChild (row ('Enabled', en));
        var co = checkbox (e.constant);
        form.appendChild (row ('Constant', co));

        var minM = input ('number', e.minMessages);
        form.appendChild (row ('Min Messages', minM));
        var selL = input ('number', e.selectiveLogic);
        form.appendChild (row ('Selective Logic', selL));

        var cmt = input ('text', e.comment);
        form.appendChild (row ('Comment', cmt));

        var ext = textarea (JSON.stringify (e.extensions || {}, null, 2));
        form.appendChild (row ('Extensions (JSON)', ext));
        ...
        function updateCounter () {
            var words, tokens;
            if (LoreMetrics && LoreMetrics.count) {
                var c = LoreMetrics.count (content.value || '');
                words = c.words;
                tokens = c.estTokens;
            } else {
        ...
        on (content, 'input', updateCounter);
        updateCounter ();
        ...
        (function () {
            ...
            for (i = 0; i < watch.length; i++) {
                on (watch[i], 'input', function () {
                    state.dirty = true;
                    renderList ();
                });
                on (watch[i], 'change', function () {
                    state.dirty = true;
                    renderList ();
                });
            }
        ...
        on (btnSave, 'click', function () {
            e.category = selCat.value;
            e.name = name.value;
            e.content = content.value;
            e.keysRaw = keys.value;
            e.keysecondaryRaw = key2.value;
            e.keyMatchPriority = !!keyPri.checked;
            e.matchWholeWords = !!whole.checked;
            e.case_sensitive = !!csens.checked;
            e.activationMode = actMode.value;
            e.activationScript = actScript.value;
            e.inclusionGroupRaw = grp.value;
            e.groupWeight = clamp (+grpW.value, 0, 1000000);
            ...
            e.constant = !!co.checked;
            e.minMessages = clamp (+minM.value, 0, 1000000);
            e.selectiveLogic = clamp (+selL.value, 0, 1000000);
            e.comment = cmt.value;
            try {
                e.extensions = JSON.parse (ext.value || '{}');
            } catch (_e) {
                alert ('Extensions JSON is invalid.');
                return;
            }
            var idx = -1, i;
            for (i = 0; i < state.entries.length; i++) if (state.entries[i].uuid === e.uuid) {
                idx = i;
                break;
            }
        ...
        on (btnClear, 'click', function () {
            selCat.value = 'uncategorized';
            name.value = '';
            content.value = '';
            keys.value = '';
            key2.value = '';
            keyPri.checked = false;
            whole.checked = true;
            csens.checked = false;
            actMode.value = 'standard';
            actScript.value = '';
            grp.value = '';
            grpW.value = 100;
            ins.value = 100;
        ...
        on (btnRemove, 'click', function () {
            ...
            var i2, out = [];
            for (i2 = 0; i2 < state.entries.length; i2++) if (state.entries[i2].uuid !== state.selectedUuid) out.push (state.entries[i2]);
            state.entries = out;
            state.selectedUuid = next;
            state.buffer = next ? clone (findByUuid (state.entries, next)) : null;
            persistAndRender ();
        });
    }

    function pickNeighborUuid (list, uuid) {
        var i, idx = -1;
        for (i = 0; i < list.length; i++) if (list[i].uuid === uuid) {
            idx = i;
            break;
        }
        if (idx === -1) return null;
        if (list.length <= 1) return null;
        return list[idx + 1] ? list[idx + 1].uuid : list[idx - 1].uuid;
    }
    ...
    function commitRender () {
        renderList ();
        renderEditor ();
        writeEntriesToCfg (state.entries);
    }
    ...
    function rerender () {
        var cfg = Store.get ();
        state.entries = readEntriesFromCfg (cfg);
        if (state.entries.length) {
            if (!state.selectedUuid) state.selectedUuid = state.entries[0].uuid;
            if (!findByUuid (state.entries, state.selectedUuid)) state.selectedUuid = state.entries[0].uuid;
            state.buffer = clone (findByUuid (state.entries, state.selectedUuid));
        } else {
    ...
    function makeSignature (c) {
        var n = (c && c.lorebook && c.lorebook.entries && c.lorebook.entries.length) || 0;
        return [n, state.selectedUuid, state.searchBy, state.searchQuery, state.compact].join ('|');
    }

    // ---- Public API ----
    var api = {};
    api.mount = function (containerEl) {
        if (!containerEl) return;
        elRoot = containerEl;
        buildSkeleton (containerEl);
        state.entries = readEntriesFromCfg (Store.get ());
        if (state.entries.length) {
            state.selectedUuid = state.entries[0].uuid;
            state.buffer = clone (state.entries[0]);
        }
        renderList ();
        renderEditor ();
        if (unsub) {
        ...
        unsub = Store.subscribe (function (c) {
