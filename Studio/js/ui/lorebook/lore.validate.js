...
(function(root){
  ...
  function entry(e, seen){
    ...
    if (cats.indexOf(e.category) === -1) { if (status !== 'err') status = 'warn'; notes.push('unknown-category'); }
    return { status: status, notes: notes };
  }

  function tally(list){
    var seen = {}; var i, v; var out = { ok:0, warn:0, err:0 };
    for (i=0;i<(list||[]).length;i++){ v = entry(list[i], seen); seen[list[i].uuid] = 1; out[v.status]++; }
    return out;
  }
