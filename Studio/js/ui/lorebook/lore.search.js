...
(function(root){
  ...
  function filterByStatus(list, status){
    var out = []; var seen = {}; var i, v;
    for(i=0;i<list.length;i++){ v = Validate.entry(list[i], seen); seen[list[i].uuid]=1; if (v.status === status) out.push(list[i]); }
    return out;
  }
