(function(root){
  ...
  function upsertEntry(entries, key){
    ...
    var e = { id:id, title:id, tags:[key.actor,key.state,key.facet], trigger:{actor:key.actor, emotion:key.state}, action:{ inject:"" }, enabled:true, priority:50, notes:"" };
    entries.push(e); return e;
  }

  // --- Seeder 1: Base descriptors
  Seed.seedActorDescriptors = function(cfg){
    var A = cfg.actors||{}, k, entries = (cfg.lorebook && cfg.lorebook.entries) || (cfg.lorebook = {entries:[]}, cfg.lorebook.entries);
    for(k in A){ if(!A.hasOwnProperty(k)) continue;
      var a = A[k];
      // Appearance
  ...
  Seed.seedEmotionCues = function(cfg){
    var A = cfg.actors||{}, keys = (root.SpeechCues && SpeechCues.keys) ? SpeechCues.keys.slice() : ['joy','sadness','anger','fear','trust','disgust','anticipation','surprise'];
    var k, i, state, entries = (cfg.lorebook && cfg.lorebook.entries) || (cfg.lorebook = {entries:[]}, cfg.lorebook.entries);
    for(k in A){ if(!A.hasOwnProperty(k)) continue;
