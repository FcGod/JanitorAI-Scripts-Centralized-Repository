...
(function(root){
  ...
  IO.import = function(rawText, existing, opts){
    existing = (existing||[]).slice(); opts = opts||{}; var overwrite = !!opts.overwrite;
    var list; var added=0, overwritten=0, skipped=0, errors=0, note='';
    try {
      var parsed = JSON.parse(rawText);
      if (Object.prototype.toString.call(parsed) === '[object Array]') list = parsed; else list = (parsed && parsed.entries) || [];
    } catch(e){ return { entries: existing, report:{ added:0, overwritten:0, skipped:0, errors:1, note:'invalid JSON' } }; }
    var byUuid = makeIndexByUuid(existing); var used = {}; var i; for(i=0;i<existing.length;i++) used[existing[i].uuid]=1;
    ...
    return { entries: existing, report:{ added:added, overwritten:overwritten, skipped:skipped, errors:errors, note:note } };
  };

  IO.export = function(entries, opts){
    opts = opts||{}; var compact = !!opts.compact; var blob = { entries: (entries||[]) };
