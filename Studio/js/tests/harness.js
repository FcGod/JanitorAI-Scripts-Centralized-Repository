...
(function(root){
  ...
  api.validate = function(cfg){
    var issues = [];
    if (!cfg) { pushIssue(issues, 'error', 'No configuration loaded'); return issues; }
    ...
    // 2. Actors
    var A = cfg.actors || [];
    if (!A.length) pushIssue(issues, 'warn', 'No actors defined', 'actors');
    else {
      var i;
      for(i=0;i<A.length;i++){
        var a = A[i] || {};
        if (!a.id) pushIssue(issues, 'error', 'Actor missing id', 'actors['+i+']');
        if (!a.displayName) pushIssue(issues, 'warn', 'Actor missing displayName', 'actors['+i+']');
        if (!a.defaults || !a.defaults.emotion) pushIssue(issues, 'error', 'Missing emotion defaults', 'actors['+i+'].defaults');
      }
    }
  ...
  try{
    ...
    else document.addEventListener('DOMContentLoaded', function(){ api.run(); });
  }catch(_e){}
})(window);
